...
type PositionedToastAddOptions = ToastManagerAddOptions<object> & {
  position?: ToastPosition;
};

type PositionedToastManager = ReturnType<typeof Toast.createToastManager> & {
  add: (options: PositionedToastAddOptions) => string;
};

/* ------------------------------ Toast Managers ------------------------------ */

const stackedToastManager =
  Toast.createToastManager() as PositionedToastManager;
const anchoredToastManager = Toast.createToastManager();

/* ------------------------------ Toast Provider ------------------------------ */

type ToasterProps = {
  variant?: "stacked" | "anchored";
...
function Toaster({
  ...
  const manager =
    variant === "anchored" ? anchoredToastManager : stackedToastManager;
  ...
  return (
    <Toast.Provider toastManager={manager}>
      ...
      {variant === "anchored" ? (
        ...
        <StackedToasts defaultPosition={defaultPosition} />
      )}
    </Toast.Provider>
  );
}
...
function StackedToasts({
  ...
  const groupedToasts = toasts.reduce<Map<ToastPosition, typeof toasts>>(
    (acc, toast) => {
      const toastPosition = ((toast as { position?: ToastPosition }).position ??
        defaultPosition ??
        "bottom-right") as ToastPosition;
      const group = acc.get(toastPosition) ?? [];
  ...
  return (
    <>
      {Array.from(groupedToasts.entries()).map(
        ([toastPosition, groupedToastsForPosition]) => {
          ...
          return (
            <Toast.Portal data-slot="toast-portal" key={toastPosition}>
              ...
              >
                {groupedToastsForPosition.map((toast) => {
                  const Icon = toast.type
                    ? ToastVariants[toast.type as ToastVariantsType]
                    : null;

                  return (
                    <Toast.Root
                      className={cn(
                        "absolute z-[calc(9999-var(--toast-index))] h-(--toast-calc-height) w-full select-none rounded-lg border bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-xs [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/6%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
                      ...
                      toast={toast}
                    >
                      <Toast.Content className="pointer-events-auto flex items-center justify-between gap-0.5 overflow-hidden px-3.5 py-3 text-sm transition-opacity duration-250 data-behind:pointer-events-none data-behind:opacity-0 data-expanded:opacity-100">
...
export { Toaster, stackedToastManager as toastManager, anchoredToastManager };
