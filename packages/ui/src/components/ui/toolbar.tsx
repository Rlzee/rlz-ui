...
function ToolbarRoot({ className, ...props }: ToolbarPrimitive.Root.Props) {
  return (
    <ToolbarPrimitive.Root
      data-slot="toolbar"
      className={cn(
        "relative flex gap-2 rounded-lg border bg-popover not-dark:bg-clip-padding p-1 text-foreground transition-colors",
        className
      )}
      {...props}
    />
  ...
  return (
    <ToolbarPrimitive.Separator
      ...
      className={cn(
        ...
        "data-[orientation=vertical]:my-1.5 data-[orientation=vertical]:w-px data-[orientation=vertical]:not-[[class^='h-']]:not-[[class*='_h-']]:self-stretch",
        className
      )}
      {...props}
    />
