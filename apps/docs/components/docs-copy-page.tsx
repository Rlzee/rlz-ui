...
function getPromptUrl(baseURL: string, url: string) {
  return `${baseURL}?q=${encodeURIComponent(
    `I’m looking at this rlz-ui documentation: ${url}

Please help me:
- Understand how to use it
- Explain key concepts
- Give practical usage examples
- Help debug issues related to this documentation

You are an expert in React UI libraries.
    `
  )}`;
}
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
  ...
  return (
    <Group>
      ...
      <Menu>
        <Menu.Trigger
          className="data-popup-open:bg-accent/70"
        ...
        >
          <Menu.Group>
            {Object.entries(menuItems).map(([key, value]) => (
              <Menu.Item
                key={key}
                className="[&_a]:flex [&_a]:gap-2 [&_a]:items-center"
              >
                {value(url)}
              </Menu.Item>
            ))}
