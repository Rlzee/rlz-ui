...
When `loading` is `true`, a spinner is appended automatically and the button is disabled.
...
Use the `loading` prop to show a spinner and disable the button during async operations.
